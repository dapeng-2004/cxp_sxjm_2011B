...
# 小偷的速度
thief_speed = 0.5  # km/min，转换为每分钟移动的百米数
thief_start_node = 32

# 小偷在t分钟后可能到达的所有节点
def thief_possible_positions(start_node, time, speed, graph):
    max_distance = time * speed * 10  # 小偷在t分钟内能走的最大距离（百米）
    return [node for node, distance in nx.single_source_dijkstra_path_length(graph, start_node).items() if distance <= max_distance]

# 小偷在3分钟后可能到达的位置
thief_positions = thief_possible_positions(thief_start_node, 3, thief_speed, city_graph)
print(f"Thief possible positions after 3 minutes: {thief_positions}")
import numpy as np

# 警察的速度
police_speed = 1.0  # km/min，转换为每分钟移动的百米数

# 计算警察从每个警察局到每个小偷可能位置的时间
...
# 计算最短路径并转换为时间
shortest_paths = {source: dict(nx.single_source_dijkstra_path_length(city_graph, source, weight='weight')) for source in police_stations.values()}
    ...
    for j, target in enumerate(nodes):
        if source != target:
            ...
            if distance != float('inf'):
                time_matrix[i, j] = distance / (police_speed * 10)  # 警察到达时间，距离单位是百米，需要转换为km
import pulp
...
# 添加约束：确保所有小偷可能位置都被警察包围
for j in thief_positions:
...
for i in range(len(police_stations)):
    for j in range(num_nodes):
        if time_matrix[i, j] > 0:  # 确保不是自身到自身的距离
...
# 获取封锁圈的边
blockade_edges = generate_circular_path(assignments, city_graph)

# 绘制最终结果
plt.figure(figsize=(15, 10))
nx.draw(city_graph, pos, with_labels=True, node_color='lightgreen', node_size=200)
nx.draw_networkx_edge_labels(city_graph, pos, edge_labels=labels)

# 绘制包围圈
if blockade_edges:
    nx.draw_networkx_edges(city_graph, pos, edgelist=blockade_edges, edge_color='r', style='dashed')
