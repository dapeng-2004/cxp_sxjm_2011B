...
\begin{abstract}
\normalsize
本文针对问题，建立了回溯算法，扩散算法，最大流二分查找，工作量函数等多种模型，解决了交巡警服务平台的设置与调度问题。

针对问题一，首先读取文件并进行数据预处理，为解决划分辖区这一问题，需满足交巡警平台管辖的范围内出现突发事件时尽量能在3分钟内抵达事发地这一条件，可将交巡警平台在三分钟内抵达该路口节点作为约束条件。以交巡警平台作为起始点向外延伸三公里进行搜索，由此根据附图1中的交通网络所给出的二十个现有的交巡警服务平台的设置情况示意图和附件2中的相关数据信息，划分管辖区域。
...
针对问题三，基于问题一已建立的扩散搜索模型，引入工作量判定函数，综合考虑A区每个交巡警平台范围内的路径长度与节点发案率，将平台工作量均衡并且尽量保证出警时间在三分钟内作为优化目标，
考虑增加不同平台数目的影响，最终确定增加平台的位置与数目，以达到最优设置方案。

针对问题四，根据对原平台部署方案的分析，其部署方案存在同一区域交巡警平台重复部署，偏远地区未部署交巡警平台等问题。分别以该市所有路口节点为中心，计算该节点三公里内的总工作量，得到由各节点总工作量组成的列表，由大到小排序后，通过计算密度函数验证其基本服从卡方分布。基于减小方差的目的每次选取平台位置都选择最靠近平均数的节点，以保证在该次选取的数据集中方差最小。选取平台后，从可选点中扣除平台所在点及其三公里所有可选点，以避免平台管辖区域的冗余。之后在重新计算剩余各点工作量的平均数，重复以上步骤，直到选出八十个平台为止。

针对问题五，基于问题二，先用扩散算法得出小偷的可能的逃跑区域，再调整递归回溯的最远距离，建立最大流模型并用二分查找进行求解，最终得出优化的围堵方案。满意结果是10.8分钟。

\textbf{关键字}：扩散搜索模型，DFS，最大流二分法，工作量函数
\end{abstract}
\newpage
...
\begin{center}
    \setlength{\tabcolsep}{9mm}{
        \begin{tabular}{ccc}
            ...
            \midrule  % 设置中间横线
            \textnormal{\(G\)} & \textnormal{} & \textnormal{}\\
            \textnormal{\(V\)} & \textnormal{节点集合} & \textnormal{单位}\\
...
平台 3 到 出入口 16 的时间: 6.0255657677970696。

平台 7 到 出入口 29 的时间: 8.015456859909348。

平台 8 到 出入口 30 的时间: 3.0608198338052887。

平台 9 到 出入口 48 的时间: 4.199410425706616。

平台 10 到 出入口 12 的时间: 7.5865852138889。

平台 11 到 出入口 23 的时间: 4.675095058668105。

平台 12 到 出入口 22 的时间: 6.882540621920882。

平台 13 到 出入口 24 的时间: 2.3853720883753127。

平台 14 到 出入口 21 的时间: 3.264965543462902。

平台 15 到 出入口 28 的时间: 4.751841748206689。

平台 16 到 出入口 14 的时间: 6.74166151627327。

平台 17 到 出入口 38 的时间: 4.7557029641094914。

平台 20 到 出入口 62 的时间: 6.448880030749422。

由此可知，都在8.015456859909348内，完成了进出该区的13条交通要道实现快速全封锁。
\subsubsection{问题的结论}
...
\subsubsection{模型求解的结果}
最优时间: 10.875649636186163min。

匹配对: [(2, '84'), (3, '85'), (5, '193'), (12, '24'), (16, '553'), (17, '90'), (19, '92'), (167, '252'), (168, '186'), (169, '254'), (172, '277'), (174, '275'), (175, '212'), (177, '175'), (178, '213'), (179, '221'), (180, '178'), (181, '274'), (182, '182'), (320, '370'), (323, '248'), (326, '371'), (372, '491'), (375, '487'), (376, '22'), (378, '530'), (383, '459'), (384, '12'), (475, '567'), (476, '476'), (477, '535'), (478, '568'), (480, '555'), (481, '554'), (482, '565'), (483, '528'), (484, '521'), (485, '563')]
...
只调用了38个交巡警服务平台，最优时间: 10.875649636186163min。
