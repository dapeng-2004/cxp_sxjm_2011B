...
针对问题三，基于问题一已建立的扩散搜索模型，引入工作量判定函数，综合考虑A区每个交巡警平台范围内的路径长度与节点发案率，将平台工作量均衡并且尽量保证出警时间在三分钟内作为优化目标，最终确定增加平台的位置与数目，以达到最优设置方案。
...
\begin{equation}
\begin{aligned}
&\lambda_1(t) = \left\{i \mid \frac{x(i, 32)}{v} \leq t, i = 1, 2, \ldots, 582 \right\} \\
&\lambda_2(t) = \left\{j \mid y(i, j) = 1, i \in \lambda_1(t) \right\} \\
&\lambda_3(t) = \lambda_2(t) - \lambda_1(t) \\
&\mu(i) = \left\{i \mid t(i, j) \leq t - 3, j \in \lambda_3(t), i = 1, 2, \ldots, 80 \right\} \\
&\min Z = t \\
&T(t) \leq t - 3 \\
&\min Z = \underset{1 \leq j \leq N}{\max} T_j \\
&\text{subject to:} \\
&\sum_{j=1}^N x_{ij} \leq 1, \quad i = 1, 2, \ldots, 80 \\
&\sum_{i=1}^{80} x_{ij} = 1, \quad j = 1, 2, \ldots, N \\
&x_{ij} \in \{0, 1\}
\end{aligned}
\end{equation}
\subsubsection{模型的求解}
...
\begin{center}
    \setlength{\tabcolsep}{9mm}{
        \begin{tabular}{cc}
            ...
            \textbf{口} & \textbf{平台}\\
            \midrule  % 设置中间横线
            \textnormal{16} & \textnormal{A16}\\
            \textnormal{41} & \textnormal{A17}\\
            \textnormal{403} & \textnormal{A2}\\
            \textnormal{361} & \textnormal{B6}\\
            \textnormal{10} & \textnormal{A10}\\
            \textnormal{44} & \textnormal{A4}\\
            \textnormal{3} & \textnormal{A18}\\
            \textnormal{40} & \textnormal{A1}\\
            \textnormal{55} & \textnormal{A3}\\
            \textnormal{60} & \textnormal{A4}\\
            \textnormal{168} & \textnormal{C10}\\
            \textnormal{4} & \textnormal{C3}\\
            \textnormal{5} & \textnormal{C7}\\
            \textnormal{6} & \textnormal{A16}\\
            \textnormal{235} & \textnormal{C8}\\
            \textnormal{171} & \textnormal{C6}\\
            \textnormal{230} & \textnormal{C7}\\
            \textnormal{273} & \textnormal{C5}\\
            \specialrule{1.2pt}{0pt}{0pt} % 设置底部粗线
        \end{tabular}
    }
\end{center}
