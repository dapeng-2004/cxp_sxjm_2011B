...
要计算警察完成附近路口任务的总工作量，我们可以定义如下的工作量函数 \( W \)，综合考虑多个因素：

\begin{equation}
    W = \sum_{i=1}^{N} \left( R_i \times T_{\text{case}} + 2 \times \frac{D_i}{V} \right)
\end{equation}

其中：
\begin{itemize}
    \item \( N \) 是路口数量
    \item \( R_i \) 是第 \( i \) 个路口的发案率
    \item \( T_{\text{case}} \) 是处理每个案件的固定时间，设为 30 分钟（0.5 小时）
    \item \( V \) 是警察行进的速度（单位：公里/小时）
    \item \( D_i \) 是从起点到第 \( i \) 个路口的距离（单位：公里）
\end{itemize}
